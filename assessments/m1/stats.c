...
/**
 * @file Assin1 
 * @brief c programming example 
 *
 * this program analyzes an array of unsigned char data items and report
    analytics on the maximum, minimum, mean, and median of the data set.
    We also have reorder this data set from large to small. 
...
void main() {
  ...
  float mean =find_mean( unsigned char test,SIZE);
  float meadian=find_median(unsigned char test,SIZE);
  unsigned char maximum=find_maximum(unsigned char test,SIZE);
  unsigned char minimum=find_minimum(unsigned char test,SIZE);
...
/* Add other Implementation File Code Her}e */
